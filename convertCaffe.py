...
def convertToCaffe(
    graph, prototxt_save_path, caffe_model_save_path, exis_focus=False, focus_concat_name=None, focus_conv_name=None
):  # 如果有 focus 层，自己添加参数
    exist_edges = []
    layers = []
    exist_nodes = []
...
def getGraph(onnx_path):
    ...
    # model = shape_inference.infer_shapes(model) passthrough注释，不然会报错
...
if __name__ == "__main__":
    ...
    onnx_path = "weights/yolov5s_sim.onnx"
    prototxt_path = "./yolov5s_conv.prototxt"
    caffemodel_path = "./yolov5s_conv.caffemodel"
    ...
    # ! 如果有focus层，记得修改convert_conv(net, node, graph, err, pass_through=0)参数为1
    # convertToCaffe(
    #     graph, prototxt_path, caffemodel_path, exis_focus=True, focus_concat_name="Concat_40", focus_conv_name="Conv_41"
    # )

    convertToCaffe(graph, prototxt_path, caffemodel_pa th)
